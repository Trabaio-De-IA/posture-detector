...
const BAD_NECK_THRESHOLD  = 30.0;  // orelha→ombro vs. vertical > 30°
const BAD_TORSO_THRESHOLD = 15.0;  // quadril→ombro vs. vertical > 15°
// Z profundidade normalizada: detecta curvatura para frente
const BAD_HEAD_FWD    = 0.80;  // cabeça > 80% da largura de ombros à frente dos ombros
// Cabeça baixa (ergonomia): mede quanto o nariz está abaixo dos olhos
// Unidade: fração da largura dos ombros
// Olhando reto para o monitor: ≈ 0.15–0.25
// Cabeça inclinada demais pra baixo: > 0.40
const BAD_HEAD_DOWN   = 0.40;
...
const SKELETON_CONNECTIONS = [
  ...
  [7, 11],  [8, 12],   // orelha → ombro (linha de referência postural)
];
...
function verticalAngle(base, tip) {
  return Math.atan2(Math.abs(tip.x - base.x), Math.abs(tip.y - base.y))
         * (180 / Math.PI);
}
...
function updateKmeansPanel(result) {
  ...
  const rows = result.centroids.map((c, i) => {
...
function detectLoop(poseLandmarker, timestamp) {
  ...
  // Usa nariz (0) vs. olho esquerdo (2) e direito (5) — landmarks de pose.
  // Quando olhando reto: nariz pouco abaixo dos olhos → headDown ≈ 0.15–0.25
  // Quando olhando muito pra baixo (tela baixa): headDown > 0.40
  ...
  // Postura ruim se QUALQUER dimensão exceder o limite
  const isBad = neckAngle  > BAD_NECK_THRESHOLD
  ...
  if (status === "Ruim") {
    ...
    if (now - lastAlertTime > ALERT_COOLDOWN_MS) {
      lastAlertTime = now;
      const msg = ALERT_MESSAGES[alertMsgIdx % ALERT_MESSAGES.length];
      alertMsgIdx++;
      speak(msg);
      if (alertInfoEl) {
        alertInfoEl.style.display = "block";
        setTimeout(() => { alertInfoEl.style.display = "none"; }, 5000);
      }
    }
  }
